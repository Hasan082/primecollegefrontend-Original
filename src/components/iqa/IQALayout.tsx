...
const IQALayout = () => {
  ...
  useEffect(() => {
    if (!user || user.role !== "iqa") navigate("/staff-login", { replace: true });
  }, [user, navigate]);

  if (!user || user.role !== "iqa") return null;
        ...
        <header className="bg-primary text-primary-foreground sticky top-0 z-30">
          <div className="px-4 sm:px-6 h-14 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold hidden sm:inline">IQA Portal</span>
            </div>
            <div className="flex items-center gap-3">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem asChild>
                    <Link to="/iqa/dashboard" className="flex items-center gap-2 cursor-pointer">
